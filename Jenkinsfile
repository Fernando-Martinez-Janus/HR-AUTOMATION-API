        ...
        stage('Deploy DEV') {
            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'registry-creds', usernameVariable: 'REG_USER', passwordVariable: 'REG_PASS')]) {
                    ...
                    bat "ssh -o StrictHostKeyChecking=no %REMOTE_USER%@%REMOTE_HOST% \"docker login %REMOTE_HOST%:5002 -u %REG_USER% -p %REG_PASS% && docker pull %REMOTE_HOST%:5002/%IMAGE_NAME%:dev && docker stop hr-api-dev; docker rm hr-api-dev; docker run -d --name hr-api-dev -p 7001:8080 %REMOTE_HOST%:5002/%IMAGE_NAME%:dev\""
                }
            }
        }
