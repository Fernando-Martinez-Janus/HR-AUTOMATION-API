...
{
    /// <summary>
    /// Provides business logic for managing rejection reasons.
    /// </summary>
    ...
    public class RejectionReasonService(
        ILogger<RejectionReasonService> logger,
    ...
    ) : IRejectionReasonService
    {
        ...
        /// </summary>
        private readonly ILogger<RejectionReasonService> _logger = logger;
        ...
        /// <summary>
        /// Defines the default expiration time used for cached rejection reason data.
        /// </summary>
        ...
        /// <summary>
        /// Defines the long expiration time used for cached rejection reason data.
        /// </summary>
        ...
        /// </summary>
        private void ValidateModel(RejectionReasonInputModel model)
        {
            ...
            ValidationResult validationResult = new RejectionReasonValidator().Validate(model);
        ...
        /// <summary>
        /// Updates the cache version and notifies the appropriate SignalR groups that rejection reasons have changed.
        /// </summary>
        ...
        private async Task HandleChangedAsync(int? organizationId = null, int? id = null)
        {
            if (id.HasValue)
            {
                string idKey = RejectionReasonCacheKeys.ById(id.Value);
            ...
            if (organizationId.HasValue)
            {
                ...
                await _cacheService.SetAsync(RejectionReasonCacheKeys.Version(organizationId.Value), CacheKeyHelper.GenerateVersion());
            }

            await _cacheService.SetAsync(RejectionReasonCacheKeys.Version(), CacheKeyHelper.GenerateVersion());
            await _notificationHub.Clients.Groups(notifyTo).SendAsync(HubKeys.RejectionReasonChanged);
        }
        ...
        public async Task<PaginationResponse<RejectionReasonViewModel>> SearchAsync(RejectionReasonSearchInputModel model)
        {
            try
            {
                int? organizationId = _httpContextService.GetOrganizationId();
                ...
                string versionKey = RejectionReasonCacheKeys.Version(model.OrganizationId);
                ...
                string searchKey = RejectionReasonCacheKeys.Search(model, version);
                PaginationResponse<RejectionReasonViewModel>? cacheResult = await _cacheService.GetAsync<PaginationResponse<RejectionReasonViewModel>>(searchKey);
                ...
                IEnumerable<RejectionReasonModel> result = await _sharedRepository.QueryAsync<RejectionReasonModel>("[config].[web_get_rejection_reasons]", parameters);

                IEnumerable<RejectionReasonViewModel> mappedResult = Mapping.Mapper.Map<IEnumerable<RejectionReasonViewModel>>(result);

                PaginationResponse<RejectionReasonViewModel> paginationResult = new(result.FirstOrDefault()?.TotalCount ?? 0, mappedResult);
        ...
        public async Task<RejectionReasonViewModel> GetAsync(int id)
        {
            try
            {
                string idKey = RejectionReasonCacheKeys.ById(id);

                RejectionReasonViewModel? cacheResult = await _cacheService.GetAsync<RejectionReasonViewModel>(idKey);
                ...
                RejectionReasonModel result =
                    await _sharedRepository.QuerySingleAsync<RejectionReasonModel>("[config].[web_get_rejection_reason_by_id]", parameters)
                    ?? throw new ResponseExceptionFactory(Exceptions.RejectionReasonNotFound);

                RejectionReasonViewModel mappedResult = Mapping.Mapper.Map<RejectionReasonViewModel>(result);
        ...
        public async Task<int> CreateAsync(RejectionReasonInputModel model)
        {
            try
            {
                ValidateModel(model);
                ...
                List<KeyValuePair<string, object?>> parameters = [
                    new("@p_organization_id", model.OrganizationId),
                    new("@p_description", model.Description),
                    new("@p_is_definitive", model.IsDefinitive),
                    new("@p_created_by", _httpContextService.GetUserId()),
                ];

                RejectionReasonModel result =
                   await _sharedRepository.QuerySingleAsync<RejectionReasonModel>("[config].[web_insert_rejection_reason]", parameters)
        ...
        public async Task UpdateAsync(int id, RejectionReasonInputModel model)
        {
            try
            {
                ValidateModel(model);
                ...
                await _sharedRepository.ExecuteAsync("[config].[web_update_rejection_reason]", parameters);
        ...
        public async Task DeleteAsync(int id)
        {
            try
            {
                ...
                RejectionReasonModel result =
                   await _sharedRepository.QuerySingleAsync<RejectionReasonModel>("[config].[web_delete_rejection_reason]", parameters)
                    ?? throw new ResponseExceptionFactory(Exceptions.InternalServerError);
