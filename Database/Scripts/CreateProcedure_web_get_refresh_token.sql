-- Retrieves a single refresh token row by its token value. Validity (expiry/revocation) is
-- re-checked application-side (RefreshTokenService.ValidateAsync) as defense in depth.

CREATE OR ALTER PROCEDURE [auth].[web_get_refresh_token]
...
BEGIN
    ...
    BEGIN TRY
        SELECT
            ...
            t.user_agent,
            t.is_enabled
        FROM auth.refresh_tokens AS t WITH (NOLOCK)
